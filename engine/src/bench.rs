...
/// search.** The SPRT harness scales every workload's time control by the speed it measures
/// from this run, so this depth chooses the length of the window that speed is read over, and a
/// short window does not only read noisily, it reads low: the run ends before the machine has
/// settled, and a side that reads low is handed a longer clock.
