...
impl<'a> Search<'a> {
    ...
    /// The root: every move, the first in the full window and the rest in a null one, the best
    /// move and its score.
            ...
            if score > best {
                ...
                if score > alpha {
                    ...
                    if alpha >= beta {
                        ...
                        // The moves it beat are the ones before it in the sorted list, which
                        // the list already holds, so nothing has to be remembered as it runs.
                        // **Some of them were never searched**, and that is stated rather than
                        // left for a reader to find: the margin above and this rule both skip
                        // moves inside this slice, so a quiet cutoff debits moves that failed
                        // to cut and moves that were given up alike.
    ...
    /// One `info` line for the iteration just completed. The pv is spelled by walking it on the
    /// board, so castling reads per the option.
